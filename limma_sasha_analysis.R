...
rownames(RG$R) <- paste(RG$genes$GeneName)
rownames(RG$G) <- paste(RG$genes$GeneName)

#RG$genes$unique <- c(1:nrow(RG))
#rownames(RG$genes) <- paste(RG$genes$unique)

#Normalize with loess method, normexp
...
# M-values #
############
# M- values after normalization/bg correction
dat <- sub.MA$M
...
rn <- rn <- rownames(topTable(fit, coef=2, n=100))
