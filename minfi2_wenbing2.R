...
colnames(Mset1) <- pd$Sample_Name
colnames(Mset1)
# [1] "BJ_1"       "BJ_hTERT_1" "BJ_SV40_1"  "BJ_Hras_1"  "EP"        
# [6] "Sen"        "BJ_2"       "BJ_hTERT_2" "BJ_SV40_2"  "BJ_Hras_2"
...
#############################################################################################
# HACKED PRE_PROC.R
navg <- cavg <- inds <- ttests <- vector("list",3)
# COMPARISONS
# [Sen, BJ_1, BJ_2] vs. EP
ind_rest <- c(6,1,7)
ind_ep <- 5
inds[[1]] <- c(ind_rest, ind_ep)
restbeta=getBeta(Mset1[,ind_rest],type="Illumina")
epbeta=getBeta(Mset1[,ind_ep],type="Illumina")
navg[[1]]=rowMeans(restbeta)
cavg[[1]]=rowMeans(epbeta)
tmpbeta1 <- cbind(restbeta,epbeta)
ttests[[1]]=genefilter::rowttests(-tmpbeta1,factor(c("R", "R", "R", "E")))
naind=which(is.na(ttests[[1]]$p.value))
if(length(naind)>0){
  cat("Fixing NAs for COMP #1","\n")
  ttests[[1]]$p.value[naind]<-1 ###for one of these we have all 0
}

# [Sen, BJ_1, BJ_2] vs. [BJ_Hras_1, BJ_Hras_2]]
ind_rest1 <- c(6,1,7)
ind_rest2 <- c(4,10)
inds[[2]] <- c(ind_rest1, ind_rest2)
rest1beta=getBeta(Mset1[,ind_rest1],type="Illumina")
rest2beta=getBeta(Mset1[,ind_rest2],type="Illumina")
navg[[2]]=rowMeans(rest1beta)
cavg[[2]]=rowMeans(rest2beta)
tmpbeta2 <- cbind(rest1beta,rest2beta)
ttests[[2]]=genefilter::rowttests(-tmpbeta2,factor(c("R1", "R1", "R1", "R2", "R2")))
naind=which(is.na(ttests[[2]]$p.value))
if(length(naind)>0){
  cat("Fixing NAs for COMP #2","\n")
  ttests[2]]$p.value[naind]<-1 ###for one of these we have all 0
}

# [Sen, BJ_1, BJ_2] vs. [BJ_hTERT_1, BJ_hTERT_2, BJ_SV40_1, BJ_SV40_2, BJ_Hras_1, BJ_Hras_2]
ind_rest3 <- c(6,1,7)
ind_rest4 <- c(2,8,3,9,4,10)
inds[[3]] <- c(ind_rest3, ind_rest4)
rest3beta=getBeta(Mset1[,ind_rest3],type="Illumina")
rest4beta=getBeta(Mset1[,ind_rest4],type="Illumina")
navg[[3]]=rowMeans(rest3beta)
cavg[[3]]=rowMeans(rest4beta)
tmpbeta3 <- cbind(rest3beta,rest4beta)
ttests[[3]]=genefilter::rowttests(-tmpbeta3,factor(c("R3", "R3", "R3", "R4", "R4", "R4", "R4", "R4", "R4")))
naind=which(is.na(ttests[[3]]$p.value))
if(length(naind)>0){
  cat("Fixing NAs for COMP #3","\n")
  ttests[[3]]$p.value[naind]<-1 ###for one of these we have all 0
}
save(cavg,navg,ttests,inds,file="Robjects/probe-results.rda")
### Figure 1 ################################################################################
...
#######################################################
## probe-level scatter plot
#######################################################
load("rdas/compTable.rda")
load("Robjects/probe-results.rda")
ann <- minfi:::.getIslandAnnotation(Mset1)
relationToIsland=ann$Relation_to_Island
type <- rep("OpenSea", length(relationToIsland))
type[relationToIsland == "Island"] <- "Island"
type[grep("Shore", relationToIsland)] <- "Shore"
type[grep("Shelf", relationToIsland)] <- "Shelf"

regionType<-factor(type,levels=c("Island","Shore","Shelf","OpenSea"))
...
# for(i in which(compTable[,3]=="cancer" & compTable[,4]=="normal")){
#   pdf(paste0("figs/scatter-",compTable[i,2],".pdf"),width=8,height=3)
  pdf(paste0("figs/scatterplot-rest1.pdf"),width=8,height=3)
  mypar(1,3)
  for(j in seq(along=Indexes)[c(1,2,4)]){
    ind=sample(Indexes[[j]],10000)
#     x=navg[[i]][ind];y=cavg[[i]][ind]
    x=navg[[1]][ind] #rest
    y=cavg[[1]][ind] #ep
    cols= 1*(abs(x-y)>0.1)+1
#     plot(x,y,cex=0.5,col=c("grey","black")[cols],pch=46,main=names(Indexes)[j],xlab="Normal",ylab=paste(compTable[i,2],"cancer"))
    plot(x,y,cex=0.5,col=c("grey","black")[cols],pch=46,main=names(Indexes)[j],xlab="Sen, BJs",ylab="EP")
    abline(0,1,col=2)
    abline(-0.1,1,lty=2,col=2)
    abline(0.1,1,lty=2,col=2)
  }
  dev.off()

pdf(paste0("figs/scatterplot-rest2.pdf"),width=8,height=3)
mypar(1,3)
for(j in seq(along=Indexes)[c(1,2,4)]){
  ind=sample(Indexes[[j]],10000)
  #     x=navg[[i]][ind];y=cavg[[i]][ind]
  ...
  cols= 1*(abs(x-y)>0.1)+1
  #     plot(x,y,cex=0.5,col=c("grey","black")[cols],pch=46,main=names(Indexes)[j],xlab="Normal",ylab=paste(compTable[i,2],"cancer"))
  plot(x,y,cex=0.5,col=c("grey","black")[cols],pch=46,main=names(Indexes)[j],xlab="Sen, BJs",ylab="BJ_Hras")
abline(0,1,col=2)
abline(-0.1,1,lty=2,col=2)
abline(0.1,1,lty=2,col=2)
}
...
pdf(paste0("figs/scatterplot-rest3.pdf"),width=8,height=3)
mypar(1,3)
for(j in seq(along=Indexes)[c(1,2,4)]){
  ind=sample(Indexes[[j]],10000)
  #     x=navg[[i]][ind];y=cavg[[i]][ind]
  ...
  cols= 1*(abs(x-y)>0.1)+1
  #     plot(x,y,cex=0.5,col=c("grey","black")[cols],pch=46,main=names(Indexes)[j],xlab="Normal",ylab=paste(compTable[i,2],"cancer"))
  plot(x,y,cex=0.5,col=c("grey","black")[cols],pch=46,main=names(Indexes)[j],xlab="Sen, BJs",ylab="BJ_hTERT, BJ_SV40, BJ_Hras")
  # [Sen, BJ_1, BJ_2] vs. [BJ_hTERT_1, BJ_hTERT_2, BJ_SV40_1, BJ_SV40_2, BJ_Hras_1, BJ_Hras_2]
  abline(0,1,col=2)
  abline(-0.1,1,lty=2,col=2)
  abline(0.1,1,lty=2,col=2)
...
# for(i in which(compTable[,3]%in%c("hyperplastic","adenoma") & compTable[,4]=="normal")){
#   pdf(paste0("figs/scatter-",compTable[i,2],"-",compTable[i,3],".pdf"),width=8,height=3)
#   mypar(1,3)
#   for(j in seq(along=Indexes)[c(1,2,4)]){
#     ind=sample(Indexes[[j]],10000)
#     x=navg[[i]][ind];y=cavg[[i]][ind]
#     cols= 1*(abs(x-y)>0.1)+1
#     plot(x,y,cex=0.3,col=c("grey","black")[cols],pch=46,main=names(Indexes)[j],xlab="Normal",ylab=paste(compTable[i,2],compTable[i,3]))
#     abline(0,1,col=2)
#     abline(-0.1,1,lty=2,col=2)
#     abline(0.1,1,lty=2,col=2)
#   }
...
beta=getBeta(cmset$object)
gr=granges(cmset$object)

# thetissues=c("breast","colon", "lung", "pancreas",  "thyroid")
group <-c("BJ","BJ_hTERT","BJ_SV40","BJ_Hras","EP","Sen")

theind<-which(compTable[,3]=="cancer" & compTable[,4]=="normal" & compTable[,2]%in%thetissues)
names(theind)<-compTable[theind,2]

###find the interesction.
openSeaIndex <- gr$type == "OpenSea"
ADD=10^6

# ind=which(pd$Tissue%in%thetissues & pd$Phenotype%in%c("normal","cancer"))
ind=c(6,1,7,5)

# Indexes=split(ind,pd$Tissue[ind])
Indexes=split(ind,pd$Sample_Group[ind])
pdf("figs/fig2.pdf",height=10,width=8)
mypar(5,2)
for(i in seq(along=theind)){
  bl=blocks[[ theind[i] ]]$table
  bl<- bsseq::data.frame2GRanges(bl,keepColumns=TRUE)
  bl <- bl[bl$value< -0.1,]
  bl <- bl[order(bl$p.value,bl$area),]
  
  s=start(bl[1,])
  e=end(bl[1,])
  ind=which(start(gr)>s-ADD & end(gr)<e+ADD &
              seqnames(gr)==as.character(seqnames(bl[1,])) & gr$type=="OpenSea")
  
  location=start(gr)[ind]
  means <-vector("list",length(Indexes))
  
  for(j in seq(along=Indexes)){
    ind2=Indexes[[j]]
    group=factor(pd$Phenotype[ind2],levels=c("normal","cancer"))
    means[[j]]=sapply(split(ind2,group), function(ind3){
      y=rowMeans(beta[ind,ind3,drop=FALSE])
      return(loess(y~location,span=
                     5*10^5/mean(diff(location))/length(location),degree=1)$fitted)})
  }
  
  
  location=location/10^6;s=s/10^6;e=e/10^6
  
  LWD<-rep(0.5,length(theind))
  LWD[i]<-2.5
  ##absolute methylation
  plot(location,beta[ind,1],ylim=c(0,1),
       xlab=paste("location on",seqnames(bl)[i],"in MB"),
       ylab="Average methylation",type="n")
  abline(v=c(s,e))
  for(j in seq(along=means))matlines(location,means[[j]],lty=c(1,2),lwd=rep(LWD[j],2),col=j)
  
  if(i==1) legend("bottomleft",names(Indexes),col=1:length(names(Indexes)),lty=1)
  
  ##difference in methyaltion
  plot(location,beta[ind,1],ylim=c(-0.25,0.25),
       xlab=paste("location on",seqnames(bl)[i],"in MB"),
       ylab="Difference methylation",type="n")
  abline(v=c(s,e))
  for(j in seq(along=means))lines(location,apply(means[[j]],1,diff),lty=1,lwd=rep(LWD[j],2),col=j)
...
# #######################################################
# ###prepare for figues
# ### this is more than we need, but during testing we looked at other 
# ###figs
# #######################################################
# 
# n<-length(Indexes[[1]])
...
# 
canceravg<-array(NA,dim=c(n,5,length(group)))
normalavg<-array(NA,dim=c(n,5,length(group)))
dists<-array(NA,dim=c(n,5))
# ##the 4 is for dist, fwer, qvalue and L
# dist2block <- array(NA,dim=c(n,length(tissues),6))
# Ns<-array(NA,dim=c(n,length(tissues)))
# N=sapply(cmset$blockInfo$indexes,length)

dist2block <- array(NA,dim=c(n,length(group),6))
Ns<-array(NA,dim=c(n,length(group)))
N=sapply(cmset$blockInfo$indexes,length)
...
cancertmp=rowMeans(betas[,1:3])
normaltmp=betas[,4]
canceravg[,3,1]<-cancertmp[Indexes[[1]]]
normalavg[,3,1]<-normaltmp[Indexes[[1]]]
...
#     normalavg[nona,k,h]<-normaltmp[ind]
    canceravg[nona,k,1]<-cancertmp[ind]
    normalavg[nona,k,1]<-normaltmp[ind]
    dists[,k]<-dd
    Ns[nona,1]<-N[ind]
  }
#   bl=bsseq::data.frame2GRanges(blocks[[thei]]$table,keepColumns=TRUE)
  bl=bsseq::data.frame2GRanges(blocks[[1]]$table,keepColumns=TRUE)
  qvs=qvalue::qvalue(bl$p.value)$q
  map<-distanceToNearest(gr[Indexes[[1]],],bl)
  dist2block[,1,1]<-values(map)$distance
  dist2block[,1,2]<-bl$fwer[subjectHits(map)]
  dist2block[,1,3]<-qvs[subjectHits(map)]
  dist2block[,1,4]<-bl$L[subjectHits(map)]
  dist2block[,1,5]<-bl$value[subjectHits(map)]
  dist2block[,1,6]<-subjectHits(map)
...
group <-c("BJ","BJ_hTERT","BJ_SV40","BJ_Hras","EP","Sen")
group
tmp2=paste(c("treated","EP","Sen"),"normal",sep="_")
tmp2=expand.grid(tmp2,tissues,stringsAsFactors = FALSE)
tmp2=cbind(tmp2,t(sapply(strsplit(tmp2[,1],"_"),function(x) x)),stringsAsFactors = FALSE)
N=rep(0,nrow(tmp2))
tmpind=tmp2[,4]=="normal"
N[tmpind]=sapply(which(tmpind),function(i)
  sum(pd$Phenotype==tmp[i,3] & pd$Tissue==tmp[i,2],na.rm=TRUE))
N[!tmpind]=sapply(which(!tmpind),function(i)
  sum(pd$Phenotype==tmp[i,4] & pd$Tissue==tmp[i,2],na.rm=TRUE))

tmp=tmp[N>0,]
tmp[,1]<-paste(tmp[,2,],tmp[,1],sep="_")
tmp <- tmp[tmp[,1]!="breast_hyperplastic_normal",]

compTable <- tmp
colnames(compTable)<-c("name","tissue","type2","type1")
#   colnames(tmp)<-c("name","tissue","type2","type1")
rownames(compTable)=1:nrow(compTable)
save(compTable,file="rdas/compTable.rda")
