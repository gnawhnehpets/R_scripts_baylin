...
target <- readTargets("/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/cohortoneandtwo/newbatch/targets_full.txt")
RG <- read.maimages(target, source="agilent", path="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/cohortoneandtwo/newbatch/")
...
colnames(objfir) <- "1-4"
colnames(objfir2) <- "1-5"
colnames(objfir3) <- "1-6"
colnames(objfir4) <- "1-7"
mafir1 <- MA.RG(objfir)
mafir2 <- MA.RG(objfir2)
mafir3 <- MA.RG(objfir3)
mafir4 <- MA.RG(objfir4)
mafir5 <- cbind(mafir1, mafir2, mafir3, mafir4)
manormfir <- normalizeWithinArrays(mafir5, method="loess", bc.method="none")
...
colnames(cy3one) <- colnames(fil_cy3)[1]

#median
cy3one2 <- as.matrix(apply(cy3fir, 1, median))
dim(cy3one2)
colnames(cy3one2) <- colnames(fil_cy3)[1]
...
cy3num <- 1
cy5num <- c(2:5)
objsec$G <- as.data.frame(sec[,1])
objsec$R <- as.data.frame(sec[,2])
objsec2$G <- as.data.frame(sec[,1])
objsec2$R <- as.data.frame(sec[,3])
objsec3$G <- as.data.frame(sec[,1])
objsec3$R <- as.data.frame(sec[,4])
objsec4$G <- as.data.frame(sec[,1])
objsec4$R <- as.data.frame(sec[,5])
colnames(objsec) <- "2-4"
colnames(objsec2) <- "2-5"
colnames(objsec3) <- "2-6"
colnames(objsec4) <- "2-7"
masec1 <- MA.RG(objsec)
masec2 <- MA.RG(objsec2)
masec3 <- MA.RG(objsec3)
masec4 <- MA.RG(objsec4)
masec5 <- cbind(masec1, masec2, masec3, masec4)
manormsec <- normalizeWithinArrays(masec5, method="loess", bc.method="none")
mabetweensec <-normalizeBetweenArrays(manormsec, method='Aquantile')
RGbsec <- RG.MA(mabetweensec)
cy3sec = RGbsec$R
head(cy3sec)
rownames(cy3sec) <- RGb$genes$GeneName
colnames(cy3sec)
cy3two <- as.matrix(rowMeans(cy3sec))
colnames(cy3two) <- colnames(fil_cy3)[2]

cy3two2 <- as.matrix(apply(cy3sec, 1, median))
dim(cy3two2)
colnames(cy3two2) <- colnames(fil_cy3)[2]
...
cy3num <- 1
cy5num <- c(2:5)
objthi$G <- as.data.frame(thi[,1])
objthi$R <- as.data.frame(thi[,2])
objthi2$G <- as.data.frame(thi[,1])
objthi2$R <- as.data.frame(thi[,3])
objthi3$G <- as.data.frame(thi[,1])
objthi3$R <- as.data.frame(thi[,4])
objthi4$G <- as.data.frame(thi[,1])
objthi4$R <- as.data.frame(thi[,5])
colnames(objthi) <- "3-4"
colnames(objthi2) <- "3-5"
colnames(objthi3) <- "3-6"
colnames(objthi4) <- "3-7"
mathi1 <- MA.RG(objthi)
mathi2 <- MA.RG(objthi2)
mathi3 <- MA.RG(objthi3)
mathi4 <- MA.RG(objthi4)
mathi5 <- cbind(mathi1, mathi2, mathi3, mathi4)
manormthi <- normalizeWithinArrays(mathi5, method="loess", bc.method="none")
mabetweenthi <-normalizeBetweenArrays(manormthi, method='Aquantile')
RGbthi <- RG.MA(mabetweenthi)
cy3thi = RGbthi$R
head(cy3thi)
rownames(cy3thi) <- RGb$genes$GeneName
colnames(cy3thi)
normfil_cy3 <- cbind(cy3fir, cy3sec, cy3thi)
dim(normfil_cy3)
cy3three <- as.matrix(rowMeans(cy3thi))
colnames(cy3three) <- colnames(fil_cy3)[3]

cy3three2 <- as.matrix(apply(cy3thi, 1, median))
colnames(cy3three2) <- colnames(fil_cy3)[3]
...
cy3num <- 1
cy5num <- c(2:4)
objfou$G <- as.data.frame(fou[,1])
objfou$R <- as.data.frame(fou[,2])
objfou2$G <- as.data.frame(fou[,1])
objfou2$R <- as.data.frame(fou[,3])
objfou3$G <- as.data.frame(fou[,1])
objfou3$R <- as.data.frame(fou[,4])
colnames(objfou) <- "1-4"
colnames(objfou2) <- "2-4"
colnames(objfou3) <- "3-4"
mafou1 <- MA.RG(objfou)
mafou2 <- MA.RG(objfou2)
mafou3 <- MA.RG(objfou3)
mafou4 <- cbind(mafou1, mafou2, mafou3)
manormfou <- normalizeWithinArrays(mafou4, method="loess", bc.method="none")
mabetweenfou <-normalizeBetweenArrays(manormfou, method='Aquantile')
RGbfou2 <- RG.MA(mabetweenfou)
cy5fou = RGbfou2$G
rownames(cy5fou) <- RGb$genes$GeneName
colnames(cy5fou)
cy5one <- as.matrix(rowMeans(cy5fou))
colnames(cy5one) <- colnames(fil_cy5)[1]

cy5one2 <- as.matrix(apply(cy5fou, 1, median))
colnames(cy5one2) <- colnames(fil_cy5)[1]
...
cy3num <- 1
cy5num <- c(2:4)
objfif$G <- as.data.frame(fif[,1])
objfif$R <- as.data.frame(fif[,2])
objfif2$G <- as.data.frame(fif[,1])
objfif2$R <- as.data.frame(fif[,3])
objfif3$G <- as.data.frame(fif[,1])
objfif3$R <- as.data.frame(fif[,4])
colnames(objfif) <- "1-5"
colnames(objfif2) <- "2-5"
colnames(objfif3) <- "3-5"
mafif1 <- MA.RG(objfif)
mafif2 <- MA.RG(objfif2)
mafif3 <- MA.RG(objfif3)
mafif4 <- cbind(mafif1, mafif2, mafif3)
manormfif <- normalizeWithinArrays(mafif4, method="loess", bc.method="none")
mabetweenfif <-normalizeBetweenArrays(manormfif, method='Aquantile')
RGbfif2 <- RG.MA(mabetweenfif)
cy5fif = RGbfif2$G
rownames(cy5fif) <- RGb$genes$GeneName
colnames(cy5fif)
cy5two <- as.matrix(rowMeans(cy5fif))
colnames(cy5two) <- colnames(fil_cy5)[2]

cy5two2 <- as.matrix(apply(cy5fif, 1, median))
colnames(cy5two2) <- colnames(fil_cy5)[2]
...
cy3num <- 1
cy5num <- c(2:4)
objsix$G <- as.data.frame(six[,1])
objsix$R <- as.data.frame(six[,2])
objsix2$G <- as.data.frame(six[,1])
objsix2$R <- as.data.frame(six[,3])
objsix3$G <- as.data.frame(six[,1])
objsix3$R <- as.data.frame(six[,4])
colnames(objsix) <- "1-6"
colnames(objsix2) <- "2-6"
colnames(objsix3) <- "3-6"
masix1 <- MA.RG(objsix)
masix2 <- MA.RG(objsix2)
masix3 <- MA.RG(objsix3)
masix4 <- cbind(masix1, masix2, masix3)
manormsix <- normalizeWithinArrays(masix4, method="loess", bc.method="none")
mabetweensix <-normalizeBetweenArrays(manormsix, method='Aquantile')
RGbsix2 <- RG.MA(mabetweensix)
cy5six = RGbsix2$G
rownames(cy5six) <- RGb$genes$GeneName
colnames(cy5six)
cy5three <- as.matrix(rowMeans(cy5six))
colnames(cy5three) <- colnames(fil_cy5)[3]

cy5three2 <- as.matrix(apply(cy5six, 1, median))
colnames(cy5three2) <- colnames(fil_cy5)[3]


# 1-3, 7
seven <- fildat[,c(1:3,7)]
objseven <- new("RGList")
objseven2 <- new("RGList")
objseven3 <- new("RGList")
cy3num <- 1
cy5num <- c(2:4)
objseven$G <- as.data.frame(seven[,1])
objseven$R <- as.data.frame(seven[,2])
objseven2$G <- as.data.frame(seven[,1])
objseven2$R <- as.data.frame(seven[,3])
objseven3$G <- as.data.frame(seven[,1])
objseven3$R <- as.data.frame(seven[,4])
colnames(objseven) <- "1-7"
colnames(objseven2) <- "2-7"
colnames(objseven3) <- "3-7"
maseven1 <- MA.RG(objseven)
maseven2 <- MA.RG(objseven2)
maseven3 <- MA.RG(objseven3)
maseven4 <- cbind(maseven1, maseven2, maseven3)
manormseven <- normalizeWithinArrays(maseven4, method="loess", bc.method="none")
mabetweenseven <-normalizeBetweenArrays(manormseven, method='Aquantile')
RGbseven2 <- RG.MA(mabetweenseven)
cy5seven = RGbseven2$G
rownames(cy5seven) <- RGb$genes$GeneName
colnames(cy5seven)
cy5four <- as.matrix(rowMeans(cy5seven))
colnames(cy5four) <- colnames(fil_cy5)[4]

cy5four2 <- as.matrix(apply(cy5seven, 1, median))
colnames(cy5four2) <- colnames(fil_cy5)[4]


normfil_cy5 <- cbind(cy5fou, cy5fif, cy5six, cy5seven)
head(normfil_cy5)
head(normfil_cy3)

############################################################################################################################
colnames(normfil_cy5)
colnames(normfil_cy3)
dim(normfil_cy5)
dim(normfil_cy3)
dim(cy3one) #pre
dim(cy3two) #post
...
dim(cy5four) #pre
# > colnames(fil_cy3)
# [1] "pre-PH1623"  "post-PH1544" "post-PH1869"
# > colnames(fil_cy5)
# [1] "pre-PH1612"  "post-PH1861" "pre-PH1632"  "pre-PH1600" 
...
save(fil_pre_col, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_pre_col")
# load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_pre_col")

save(fil_post_col, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_post_col")
# load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_post_col")

save(fil_pre_med_col, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_pre_med_col")
# load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_pre_med_col")

save(fil_post_med_col, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_post_med_col")
...
genename
save(genename, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_genename")
# load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_post_med_col")
...
f <- factor(as.character(sub("-\\w+$", "", colnames(fil_test_med2)))) # for all samples, new.dat
f
library(limma)
pval <- .2
design <- model.matrix(~f)
design
fit <- eBayes(lmFit(log2(fil_test_med2), design))
selected  <- fit$p.value[, 2] < pval # or p<.01 #p.adjust(fit$p.value[, 2], method="BY")<.05
esetSel <- fil_test[selected,]
dim(esetSel)
head(esetSel)
