...
# S 2   pre-PH1550  ***pre-PH1600***
# ###R3   pre-JHH005 post-JHH005
# ###R4   pre-PH1604  pre-PH1606
# S 5  ***post-PH1544***  pre-PH1544
# S 6   pre-PH1622  ***pre-PH1631***
# ###R7   pre-PH1545 post-PH1545
# S 8   pre-PH1623  pre-PH1632
# ###R9  post-PH1815  pre-PH1815
# ###R10   pre-PH1635  pre-PH1644
# ###R11  pre-PH1827 post-PH1827
# ###R12  pre-PH1811  pre-PH1816
# ###R13  pre-PH1636 post-PH1640
# ###R14  pre-PH1892  pre-PH1902
# x 15 post-PH1612  ***pre-PH1612***
# ###R16  pre-PH1868  pre-PH1887
# ###R17  pre-PH1616 post-PH1616
# ###R18 post-PH1910  pre-PH1910
# ###R19  pre-PH1900 post-PH1900
# ###R20 post-PH1843  pre-PH1843
# ###R21  pre-PH1913  pre-PH1886
# ###R22 post-PH1844  pre-PH1844
# x 23 pre-PH1861 ***post-PH1861***
# S 24 ***post-PH1869***  pre-PH1869
# ###R25 pre-PH1871 post-PH1871
# ###R26 post-PH1876  pre-PH1876
# P 27 pre-PH1545 post-PH1545
# P 28 post-PH1565  pre-PH1565
# P 29 post-PH1910  pre-PH1910
# S 30 pre-PH1641 post-PH1640
# P 31 ***post-PH1612***  pre-PH1824
# P 32 ***pre-PH1861*** post-PH1900

rm(list=ls())
for(i in 1:20){gc()}
library(limma)

setwd("/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/")
target <- readTargets("/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/targets_full.txt")
#RG <- read.maimages(target, source="agilent", path="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/")
# save(RG, file="./R objects/RG")
load("./R objects/RG")
...
# POSTLESLIE
pos <- c(2,5,6,8,15,23,24,27,28,29,30,31,32)
dat <- dat[,pos]
colnames(dat)
dim(dat)
targets <- targets[pos,]
...
**********************************
#RUN FOR PAIRS ONLY###############
dat <- dat[,c(5,6,8,9,10)]
targets <-targets[c(5,6,8,9,10),]
##################################
**********************************
...
limma::plotMA3by2(dat, prefix="prenorm_MA_newbatch_filtered")
...
# f$R[is.na(f$R)]
# f$Rb[is.na(f$Rb)]
# f$G <- ifelse(f$R>0, f$G, 100)
# f$Gb <- ifelse(f$Rb>0, f$Gb, 100)
# f$R <- ifelse(f$G>0, f$R, 100)
# f$Rb <- ifelse(f$Gb>0, f$Rb, 100)
...
plotDensities(normwithin, main = "RG densities - normwithin - none/none/aquantile")
#plotDensities(normwithin, main = "RG densities - normwithin - loess/none/aquantile")
...
plotDensities(normbetween, main = "RG densities - normbetween - none/none/aquantile")
# plotDensities(normbetween, main = "RG densities - normbetween - loess/none/aquantile")
#dev.off()
...
# limma::plotMA3by2(RGb, prefix="MA_filtered_nonorm")
# limma::plotMA3by2(RGb, prefix="MA_filtered_loess")
# limma::plotMA3by2(RGb, prefix="MA_leslie_loess")
limma::plotMA3by2(RGb, prefix="MA_newbatch_nonorm")
limma::plotMA3by2(RGb, prefix="MA_newbatch_loess")
getwd()
# head(RGb$G)
# names(RGb)
...
rownames(cy3) <- RGb$genes$GeneName
colnames(cy3) <- paste(targets$Cy3, targets$Cy3_Sample, sep="-")
data.frame(colnames(cy3))
cy5 = RGb$G
rownames(cy5) <- RGb$genes$GeneName
colnames(cy5) <- paste(targets$Cy5, targets$Cy5_sample, sep="-")
data.frame(colnames(cy5))
data.frame(colnames(cy3), colnames(cy5))

######################## DATA OBJECT PROCESSING ########################
# PAIRS ONLY
# fil_cy3 <- cy3[,c(12,13,8:10)]
# fil_cy5 <- cy5[,c(5,6,8:10)]
fil_cy3 <- cy3
fil_cy5 <- cy5
data.frame(colnames(fil_cy3),colnames(fil_cy5))
pre <- cbind(fil_cy5[,1], fil_cy3[,c(2,3)], fil_cy5[,c(4,5)])
post <- cbind(fil_cy3[,1], fil_cy5[,c(2,3)], fil_cy3[,c(4,5)])
colnames(pre) <- sub("post", "pre", colnames(fil_cy3))
colnames(post) <- sub("pre", "post", colnames(fil_cy3))
dim(pre)
colnames(post)
pairs <- cbind(pre, post)
head(pairs)
pairs <- NULL
colnamespairs <- NULL
pairs
for(i in 1:5){
  pairs <- cbind(pairs, pre[,i], post[,i])  
  colnamespairs <- c(colnamespairs, colnames(pre)[i], colnames(post)[i])
}
colnames(pairs) <- colnamespairs
colnames(pairs)
head(pairs)
rgpairs <- new("RGList")
rgpairs$G <- pre
rgpairs$R <- post
normpairs1 <- normalizeWithinArrays(rgpairs, method="loess", bc.method="none")
normpairs2 <-normalizeBetweenArrays(normpairs1, method='Aquantile')
RGbpairs <- RG.MA(normpairs2)
pairs_pre = RGbpairs$R
rownames(pairs_pre) <- rownames(pre)
colnames(pairs_pre) <- colnames(pre)
head(pairs_pre)
data.frame(colnames(pairs_pre))
pairs_post = RGbpairs$G
rownames(pairs_post) <- rownames(post)
colnames(pairs_post) <- colnames(post)
data.frame(colnames(pairs_post))
data.frame(colnames(pairs_pre), colnames(pairs_post))
pairs_norm <- NULL
colnamespairs_norm <- NULL
for(i in 1:5){
  pairs_norm <- cbind(pairs_norm, pairs_pre[,i], pairs_post[,i])  
  colnamespairs_norm <- c(colnamespairs_norm, colnames(pairs_pre)[i], colnames(pairs_post)[i])
}
colnames(pairs_norm) <- colnamespairs_norm
head(pairs_norm)
dim(pairs_norm)
# save(pairs_norm, file="./R objects/pairs_norm") #12/2014

rassf1_pair <- pairs_norm["RASSF1",]
birc5_pair <- pairs_norm["BIRC5",]
head(rassf1_pair, n=20)
birc5_pair

intersect(intersect(intersect(genes5, uniquegenes), difexp), aim)
data.frame(intersect(intersect(genes5, uniquegenes), difexp))

paireddat <- rbind(birc5_pair, rassf1_pair)
...
ht <- heatmap.2(paireddat,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row", Colv = NA)
ht <- heatmap.2(paireddat,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(paireddat,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(log2(paireddat), main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
head(ht)

limma::plotMA3by2(rgdat, prefix="rgdat_nonorm")
limma::plotMA3by2(normat2, prefix="rgdat_norm")
getwd()
# ALL SAMPLES

cy3         cy5
1 S 2   pre-PH1550  ***pre-PH1600***
2 S 5  ***post-PH1544***  pre-PH1544
3 S 6   pre-PH1622  ***pre-PH1631***
4 S 8   pre-PH1623***  ***pre-PH1632
5 x 15 post-PH1612  ***pre-PH1612***
6 x 23 pre-PH1861 ***post-PH1861***
7 S 24 ***post-PH1869***  pre-PH1869
8 P 27 pre-PH1545 post-PH1545
9 P 28 post-PH1565  pre-PH1565
10 P 29 post-PH1910  pre-PH1910
11 S 30 pre-PH1641 post-PH1640
12 P 31 ***post-PH1612***  pre-PH1824
13 P 32 ***pre-PH1861*** post-PH1900


fil_cy3 <- cy3[,c(2,4,7:11,12,13)]
fil_cy5 <- cy5[,c(1,3:6,8:11)]
data.frame(colnames(fil_cy3),colnames(fil_cy5))
fildat <- cbind(fil_cy3, fil_cy5)
colnames(fildat)
dim(fildat)

save(fildat, file="./R objects/fildat")
load("./R objects/fildat")
...
for(i in 1:9){
  ...
  cy3num <- 1
  cy5num <- c(2:10)
  objfir$G <- as.data.frame(fir[,1])
  objfir$R <- as.data.frame(fir[,2])
  objfir2$G <- as.data.frame(fir[,1])
  objfir2$R <- as.data.frame(fir[,3])
  objfir3$G <- as.data.frame(fir[,1])
  objfir3$R <- as.data.frame(fir[,4])
  objfir4$G <- as.data.frame(fir[,1])
  objfir4$R <- as.data.frame(fir[,5])
  objfir5$G <- as.data.frame(fir[,1])
  objfir5$R <- as.data.frame(fir[,6])
  objfir6$G <- as.data.frame(fir[,1])
  objfir6$R <- as.data.frame(fir[,7])
  objfir7$G <- as.data.frame(fir[,1])
  objfir7$R <- as.data.frame(fir[,8])
  objfir8$G <- as.data.frame(fir[,1])
  objfir8$R <- as.data.frame(fir[,9])
  objfir9$G <- as.data.frame(fir[,1])
  objfir9$R <- as.data.frame(fir[,10])
  
  colnames(objfir) <- paste(i, 10, sep = "-")
  colnames(objfir2) <- paste(i, 11, sep = "-")
  colnames(objfir3) <- paste(i, 12, sep = "-")
  colnames(objfir4) <- paste(i, 13, sep = "-")
  colnames(objfir5) <- paste(i, 14, sep = "-")
  colnames(objfir6) <- paste(i, 15, sep = "-")
  colnames(objfir7) <- paste(i, 16, sep = "-")
  colnames(objfir8) <- paste(i, 17, sep = "-")
  colnames(objfir9) <- paste(i, 18, sep = "-")
  
  mafir1 <- MA.RG(objfir)
  mafir2 <- MA.RG(objfir2)
  mafir3 <- MA.RG(objfir3)
  mafir4 <- MA.RG(objfir4)
  mafir5 <- MA.RG(objfir5)
  mafir6 <- MA.RG(objfir6)
  mafir7 <- MA.RG(objfir7)
  mafir8 <- MA.RG(objfir8)
  mafir9 <- MA.RG(objfir9)
  mafir10 <- cbind(mafir1, mafir2, mafir3, mafir4, mafir5, mafir6, mafir7, mafir8, mafir9)
  manormfir <- normalizeWithinArrays(mafir10, method="loess", bc.method="none")
  mabetweenfir <-normalizeBetweenArrays(manormfir, method='Aquantile')
  RGbfir2 <- RG.MA(mabetweenfir)
  cy3fir = RGbfir2$R
  rownames(cy3fir) <- RGb$genes$GeneName
  colnames(cy3fir)
  head(cy3fir)
  cy3one <- as.matrix(rowMeans(cy3fir))
  colnames(cy3one) <- colnames(fil_cy3)[i]
  
  #median
  cy3one2 <- as.matrix(apply(cy3fir, 1, median))
  dim(cy3one2)
  colnames(cy3one2) <- colnames(fil_cy3)[i]
  head(cy3one2)
  assign(paste("cy5var", i, sep=""), cy3one)
  assign(paste("cy5var_med", i, sep=""), cy3one2)
}
...
for(i in 1:9){
  ...
  head(cy3fir)
  cy3one <- as.matrix(rowMeans(cy3fir))
  colnames(cy3one) <- colnames(fil_cy5)[i]
  
  #median
  cy3one2 <- as.matrix(apply(cy3fir, 1, median))
  dim(cy3one2)
  colnames(cy3one2) <- colnames(fil_cy5)[i]
  head(cy3one2)
  assign(paste("cy3var", i, sep=""), cy3one)
  assign(paste("cy3var_med", i, sep=""), cy3one2)
}


names1 <- c(colnames(cy3var1),
colnames(cy3var2),
colnames(cy3var3),
colnames(cy3var4),
colnames(cy3var5),
...
data.frame(colnames(fil_cy3),names2)
data.frame(colnames(fil_cy5),names1)
data.frame(names1, names2)


normfil_cy3 <- cbind(cy3var1, #pre
                     cy3var2, #pre
                     cy3var3, #pre
                     cy3var4, #pre
                     cy3var5, #post
...
normfil_cy5 <- cbind(cy5var1, #post
                     cy5var2, #pre
                     cy5var3, #post
                     cy5var4, #pre
                     cy5var5, #post
...
normfil_cy3_med <- cbind(cy3var_med1, #pre
                     cy3var_med2, #pre
                     cy3var_med3, #pre
                     cy3var_med4, #pre
                     cy3var_med5, #post
...
normfil_cy5_med <- cbind(cy5var_med1, #post
                     cy5var_med2, #pre
                     cy5var_med3, #post
                     cy5var_med4, #pre
                     cy5var_med5, #post
...
pre <- cbind(normfil_cy3[,c(1:4,7,8)],normfil_cy5[,c(2,4,7,9)])
post <- cbind(normfil_cy3[,c(5,6,9)], normfil_cy5[,c(1,3,5,6,8)])
pre_med <- cbind(normfil_cy3_med[,c(1:4,7,8)],normfil_cy5_med[,c(2,4,7,9)])
post_med <- cbind(normfil_cy3_med[,c(5,6,9)], normfil_cy5_med[,c(1,3,5,6,8)])

#reconfigure pre/post objects so that five pairs are first five samples in both objects
fil_pre <- pre[,c(10,8,5,6,4,1:3,7,9)]
fil_post <- post[,c(1,2,6,7,8,3:5)]
fil_pre_med <- pre_med[,c(10,8,5,6,4,1:3,7,9)]
fil_post_med <- post_med[,c(1,2,6,7,8,3:5)]

data.frame(colnames(fil_pre),colnames(fil_pre_med))
data.frame(colnames(fil_post), colnames(fil_post_med))

dim(fil_pre)
dim(fil_post)

colnames(fil_pre_med)
colnames(pre_med)
colnames(fil_post_med)
colnames(post_med)
dim(fil_pre_med)
dim(fil_post_med)
save(pre, file="./R objects/pre")
save(post, file="./R objects/post")
save(pre_med, file="./R objects/pre_med")
save(post_med, file="./R objects/post_med")
save(fil_pre, file="./R objects/fil_pre")
save(fil_post, file="./R objects/fil_post")
save(fil_pre_med, file="./R objects/fil_pre_med")
save(fil_post_med, file="./R objects/fil_post_med")
############################################################################################################################

#mean
#fil_test <- cbind(cy3one, cy3two, cy3three, cy5one, cy5two, cy5three, cy5four)
#fil_pre <- cbind(cy3one, cy5one, cy5three, cy5four)
#fil_post <- cbind(cy3two, cy3three, cy5two)

fil_pre_col<- apply(fil_pre,2,function(v){tapply(v,names(v),function(x){median(x,na.rm=TRUE)})})
fil_post_col<- apply(fil_post,2,function(v){tapply(v,names(v),function(x){median(x,na.rm=TRUE)})}) 

#median
# fil_test_med <- cbind(cy3one2, cy3two2, cy3three2, cy5one2, cy5two2, cy5three2, cy5four2)
# fil_pre_med <- cbind(cy3one2, cy5one2, cy5three2, cy5four2)
# fil_post_med <- cbind(cy3two2, cy3three2, cy5two2)

fil_pre_med_col<- apply(fil_pre_med,2,function(v){tapply(v,names(v),function(x){median(x,na.rm=TRUE)})})
fil_post_med_col<- apply(fil_post_med,2,function(v){tapply(v,names(v),function(x){median(x,na.rm=TRUE)})}) 

# total
# fil_test2<- apply(fil_test,2,function(v){tapply(v,names(v),function(x){median(x,na.rm=TRUE)})}) 
# fil_test_med2<- apply(fil_test_med,2,function(v){tapply(v,names(v),function(x){median(x,na.rm=TRUE)})}) 

# save(fil_pre_col, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/fil_pre_col")
load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_pre_col")

# save(fil_post_col, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/fil_post_col")
load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_post_col")

# save(fil_pre_med_col, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/fil_pre_med_col")
load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_pre_med_col")

# save(fil_post_med_col, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/fil_post_med_col")
load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_post_med_col")

fil_total <- cbind(fil_pre_col, fil_post_col)
fil_total_med <- cbind(fil_pre_med_col, fil_post_med_col)
# save(fil_total, file="./R objects/fil_total")
# save(fil_total_med, file="./R objects/fil_total_med")
load(file="./R objects/fil_total")
load(file="./R objects/fil_total_med")
cbind(head(fil_total[,1:3]),head(fil_total_med[,1:3]))

genename <- rownames(fil_pre_col)
genename
save(genename, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/fil_genename")
# load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/m084b trial/fil_post_med_col")
...
head(fil_total)
head(fildat)
pairs_norm_condensed<- apply(pairs_norm,2,function(v){tapply(v,names(v),function(x){median(x,na.rm=TRUE)})})

# Differentially expressed genes
x data <- fil_total
x data <- fil_total_med
data <- fildat
data <- pairs_norm_condensed #12/2014
# rassf1 <- fildat["RASSF1",]
# birc5 <- fildat["BIRC5",]
# rassf1
# birc5
# rassf1 <- fil_total["RASSF1",]
# birc5 <- fil_total["BIRC5",]
# rassf1
# birc5
# rassf1 <- fil_total_med["RASSF1",]
# birc5 <- fil_total_med["BIRC5",]
# rassf1
# birc5

f <- factor(as.character(sub("-\\w+$", "", colnames(data)))) # for all samples, new.dat
f
length(f)
library(limma)
pval <- .05
...
design
fit <- eBayes(lmFit(log2(data), design))
fit <- eBayes(lmFit(data, design))
# save(fit, file="./R objects/difexp2_fit")
...
dim(esetSel)
head(esetSel)
data.frame(rownames(esetSel))
difexp <- rownames(esetSel)
print(difexp, quote = FALSE)
length(difexp)

save(difexp2, file="./R objects/difexp2")

load(file="./R objects/exp_fc14")
difexp <- exp_fc14
intersect(difexp, difexp2)
sort(difexp2)
sort(difexp)

load("./R objects/genes5")
load("./R objects/genes3")
load("./R objects/uniquesig_dmp")
load("./R objects/uniquesig_dmp2")
load("./R objects/uniquegenes")
load("./R objects/aim")
length(genes3)
length(genes5)
length(uniquesig_dmp)
length(aim)
length(intersect(aim, difexp))
head(log2(filtered))

intersect(uniquesig_dmp, uniquesig_dmp2)
length(intersect(genes5, difexp))
intersect(genes5, difexp)
intersect(intersect(genes5, uniquegenes),aim)
length(intersect(uniquegenes, difexp))
intersect(intersect(uniquegenes, difexp),aim)
intersect(aim, intersect(genes5, difexp))
intersect(aim, intersect(uniquegenes, difexp))

#cbind(fil_total_med[difexp,1],fil_total[difexp,1])

difexp
difmetdata <- read.table("genelistqval05.txt", header=FALSE)
intersect(difexp, difmetdata[,2])
difmetdata <- read.table("./updatedval05.txt", header=TRUE)
difmetdata <- read.table("./uniquegenesqval05.txt", header=TRUE)
dim(difmetdata)
difmetdata
#difmet <- as.character(difmetdata[,2])
difmet <- as.character(difmetdata[,1])
difmet
print(intersect(difexp, difmet), quote=FALSE)
getwd()
...
cta
hervs
print(intersect(aim, difexp), quote=FALSE)

# Differential expression of patients with survival past 6 months of treatment
# difexp = rownames(esetSel)
# which(rownames(esetSel) == difexp)
dim(esetSel)
colnames(esetSel)
# unsupervised
library(gplots)
...
#******************************************************************************************************************************************
heatmapdat <- esetSel
data.frame(colnames(esetSel[,c(1:5)]), colnames(esetSel[,c(11:15)]))
dim(esetSel)
#all samples
filtered <- heatmapdat
library(gplots)
title <- "all samples, differentially expressed"
...
ht <- heatmap.2(filtered,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row", Colv = NA)
ht <- heatmap.2(filtered,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(filtered,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(log2(filtered), main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
names(ht)
head(ht)
# pre-only #NEED TO SELECT 'pos' FOR ALL SAMPLES
# need: filtered <- heatmapdat
# NOT NECESSARY??
colnames(filtered[,c(1:10)])
ht <- heatmap.2(filtered[,c(1:10)],       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row", Colv = NA)
ht <- heatmap.2(log2(filtered[,c(1:10)]), main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
...
# pairs
heatmappairs <- esetSel[,c(1:5,11:15)]

#two aim genes
heatmappairs <- esetSel[c("CXCL6", "SCG2"),c(1:5,11:15)]
filtered <- heatmappairs

title <- "pairs only, differentially expressed"
...
# number of samples with over 6 months survival
# grab data for patients over six months, vs patients under six months survival

#allsamples
title <- "all samples, 6mo survival"
length(meta$patient[meta$sixmonthsoverall == 1])
moresixsamples <- as.character(meta$patient[meta$sixmonthsoverall == 1])
moresixsamples
lesssixsamples <- as.character(meta$patient[meta$sixmonthsoverall == 0])
lesssixsamples

moresix <- which(colnames(esetSel) %in% moresixsamples)
lesssix <- which(colnames(esetSel) %in% lesssixsamples)
moresix
lesssix
dim(esetSel)
colnames(esetSel)

neweset <- cbind(esetSel[,moresix], esetSel[,lesssix])
paste("M", colnames(esetSel[,moresix]), sep="-")
paste("L", colnames(esetSel[,lesssix]), sep="-")
colnames(neweset) <- c(paste("M", colnames(esetSel[,moresix]), sep="-"), paste("L", colnames(esetSel[,lesssix]), sep="-"))
head(neweset)
...
ht <- heatmap.2(neweset,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1.1, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row", Colv = NA)
ht <- heatmap.2(neweset,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1.1, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(neweset,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(log2(neweset), main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")

#pres only
title <- "all samples, pres only, 6mo survival"
length(meta$patient[meta$sixmonthsoverall == 1])
moresixsamples <- as.character(meta$patient[meta$sixmonthsoverall == 1])
moresixsamples[sub("-.*", "", moresixsamples)=="pre"]
moresixpre <- which(colnames(esetSel) %in% moresixsamples[sub("-.*", "", moresixsamples)=="pre"])
lesssixsamples <- as.character(meta$patient[meta$sixmonthsoverall == 0])
lesssixsamples[sub("-.*", "", lesssixsamples)=="pre"]
lesssixpre <- which(colnames(esetSel) %in% lesssixsamples[sub("-.*", "", lesssixsamples)=="pre"])

newesetpre <- cbind(esetSel[,moresixpre], esetSel[,lesssixpre])
paste("M", colnames(esetSel[,moresixpre]), sep="-")
paste("L", colnames(esetSel[,lesssixpre]), sep="-")
colnames(newesetpre) <- c(paste("M", colnames(esetSel[,moresixpre]), sep="-"), paste("L", colnames(esetSel[,lesssixpre]), sep="-"))
head(newesetpre)
...
ht <- heatmap.2(newesetpre,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row", Colv = NA)
ht <- heatmap.2(newesetpre,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(newesetpre,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(log2(newesetpre), main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")

#pairs only
title <- "pairs only, 6mo survival"
heatmappairs <- esetSel[,c(1:5,11:15)]
...
moresixpairs <- which(colnames(heatmappairs) %in% moresixsamples)
lesssixpairs <- which(colnames(heatmappairs) %in% lesssixsamples)
moresixpairs
lesssixpairs
dim(esetSel)
colnames(esetSel)

newesetpairs <- cbind(heatmappairs[,moresixpairs], heatmappairs[,lesssixpairs])
paste("M", colnames(heatmappairs[,moresixpairs]), sep="-")
paste("L", colnames(heatmappairs[,lesssixpairs]), sep="-")
colnames(newesetpairs) <- c(paste("M", colnames(heatmappairs[,moresixpairs]), sep="-"), paste("L", colnames(heatmappairs[,lesssixpairs]), sep="-"))
head(neweset)
...
ht <- heatmap.2(newesetpairs,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1.1, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row", Colv = NA)
ht <- heatmap.2(newesetpairs,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= 1.1, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(newesetpairs,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(log2(newesetpairs), main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")

# 9 MONTHS

# number of samples with over 9 months survival
# grab data for patients over nine months, vs patients under nine months survival

#allsamples
title <- "all samples, 9mo survival"
length(meta$patient[meta$ninemonthsoverall == 1])
moreninesamples <- as.character(meta$patient[meta$ninemonthsoverall == 1])
moreninesamples
lessninesamples <- as.character(meta$patient[meta$ninemonthsoverall == 0])
lessninesamples

morenine <- which(colnames(esetSel) %in% moreninesamples)
lessnine <- which(colnames(esetSel) %in% lessninesamples)
morenine
lessnine
dim(esetSel)
colnames(esetSel)

neweset <- cbind(esetSel[,morenine], esetSel[,lessnine])
paste("M", colnames(esetSel[,morenine]), sep="-")
paste("L", colnames(esetSel[,lessnine]), sep="-")
colnames(neweset) <- c(paste("M", colnames(esetSel[,morenine]), sep="-"), paste("L", colnames(esetSel[,lessnine]), sep="-"))
head(neweset)
...
ht <- heatmap.2(neweset,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row", Colv = NA)
ht <- heatmap.2(neweset,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(neweset,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(log2(neweset), main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")

#pres only
title <- "all samples, pres only, 9mo survival"
length(meta$patient[meta$ninemonthsoverall == 1])
moreninesamples <- as.character(meta$patient[meta$ninemonthsoverall == 1])
moreninesamples[sub("-.*", "", moreninesamples)=="pre"]
moreninepre <- which(colnames(esetSel) %in% moreninesamples[sub("-.*", "", moreninesamples)=="pre"])
lessninesamples <- as.character(meta$patient[meta$ninemonthsoverall == 0])
lessninesamples[sub("-.*", "", lessninesamples)=="pre"]
lessninepre <- which(colnames(esetSel) %in% lessninesamples[sub("-.*", "", lessninesamples)=="pre"])

newesetpre <- cbind(esetSel[,moreninepre], esetSel[,lessninepre])
paste("M", colnames(esetSel[,moreninepre]), sep="-")
paste("L", colnames(esetSel[,lessninepre]), sep="-")
colnames(newesetpre) <- c(paste("M", colnames(esetSel[,moreninepre]), sep="-"), paste("L", colnames(esetSel[,lessninepre]), sep="-"))
head(newesetpre)
...
moreninepairs <- which(colnames(heatmappairs) %in% moreninesamples)
lessninepairs <- which(colnames(heatmappairs) %in% lessninesamples)
moreninepairs
lessninepairs
dim(esetSel)
colnames(esetSel)

newesetpairs <- cbind(heatmappairs[,moreninepairs], heatmappairs[,lessninepairs])
paste("M", colnames(heatmappairs[,moreninepairs]), sep="-")
paste("L", colnames(heatmappairs[,lessninepairs]), sep="-")
colnames(newesetpairs) <- c(paste("M", colnames(heatmappairs[,moreninepairs]), sep="-"), paste("L", colnames(heatmappairs[,lessninepairs]), sep="-"))
head(neweset)
...
ht <- heatmap.2(newesetpairs,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row", Colv = NA)
ht <- heatmap.2(newesetpairs,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(newesetpairs,       main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("none"), symm=F,symkey=F,symbreaks=T, scale="row")
ht <- heatmap.2(log2(newesetpairs), main=title, col=mycolor, breaks=colbreak, density.info="none", trace="none", srtCol = 45, margins=c(7,10), cexCol= .7, offsetCol = -.6, dendrogram=c("both"), symm=F,symkey=F,symbreaks=T, scale="row")
...
# Heatmap
new.datfil <- fil_test2
new.datfil <- fil_test_med2
#new.datfil <- esetSel
dim(new.datfil)
one<-new.datfil["ADM",]
...
dim(dat)
stats <- paste(nrow(dat), ncol(dat), sep="\t")
write.table("#1.2", file = filename, append=FALSE, quote=FALSE, col.names=FALSE, row.names=FALSE)
write.table(stats, file = filename, append=TRUE, quote=FALSE, col.names=FALSE, row.names=FALSE)
header <- cbind(rownames(dat), matrix(data=NA, nrow=nrow(dat), ncol=1), dat)
colnames(header)[1]<- "NAME"
colnames(header)[2]<- "Description"
write.table(header, file = filename, sep="\t", row.names=FALSE, append=TRUE)

class <- factor(as.character(sub("-\\w+$", "", colnames(dat))))
classes <- unique(class)
length(classes)
header <- paste(ncol(dat), length(classes), 1, sep=" ")
write.table(header, file = filename2, append=FALSE, quote=FALSE, col.names=FALSE, row.names=FALSE)
line2 <- paste("#", classes[1], classes[2], sep=" ")
write.table(line2, file = filename2, append=TRUE, quote=FALSE, col.names=FALSE, row.names=FALSE)
class <- toString(class)
class <- gsub(",", "", class)
write.table(as.character(class), file = filename2, append=TRUE, quote=FALSE, col.names=FALSE, row.names=FALSE)
...
# PLOTS ####################################
############################################
allsamp <- cbind(normfil_cy3_med, normfil_cy5_med)
dat.cor <- cor(allsamp)
...
rname <- as.matrix(dat$genes$GeneName[position])
dim(rname)
head(rname)
fc <- as.matrix(edat$logFC)
index_tt <- which(abs(fc)>1.4)
exp_fc14 <- unique(edat$GeneName[index_tt])
length(exp_fc142)
index_tt <- which(abs(fc)>2)
exp_fc2 <- unique(edat$GeneName[index_tt])


index_tt <- which(abs(fc)>1.4)
exp_fc142 <- unique(edat$GeneName[index_tt])
length(exp_fc142)
index_tt <- which(abs(fc)>2)
exp_fc22 <- unique(edat$GeneName[index_tt])
save(exp_fc14, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/exp_fc14")
save(exp_fc142, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/exp_fc142")
save(exp_fc2, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/exp_fc2")
save(exp_fc22, file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/exp_fc22")
load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/exp_fc14")
load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/exp_fc142")
load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/exp_fc2")
load(file="/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/updatedbatch/R objects/exp_fc22")
load(file="Z:/users/shwang26/updatedbatch/R objects/exp_fc14")
load(file="Z:/users/shwang26/updatedbatch/R objects/exp_fc2")
load(file="Z:/users/shwang26/updatedbatch/R objects/uniquesig_dmp")
load(file="Z:/users/shwang26/updatedbatch/R objects/exp_fc2")
length(uniquesig_dmp)
length(exp_fc14)
length(exp_fc2)

sort(intersect(aim, exp_fc14)) #3
intersect(intersect(genes3, uniquesig_dmp), aim) #4
intersect(exp_fc14, genes3) #5
intersect(uniquesig_dmp, exp_fc14) #6
intersect(intersect(genes3, uniquesig_dmp), exp_fc14) #7
intersect(intersect(intersect(genes3, uniquesig_dmp), exp_fc14), aim)
intersect(genes3, cta)
intersect(genes3, hervs)
intersect(uniquesig_dmp, cta)
intersect(uniquesig_dmp, hervs)
intersect(exp_fc14, cta)
intersect(exp_fc14, hervs)
intersect(aim, cta)
intersect(aim, hervs)
...
intersect(aim, minfi) #1
print(sort(intersect(aim, genes3)), quote=FALSE) #2
intersect(aim, difexp) #3
print(sort(intersect(lab, minfi)), quote=FALSE) #4
intersect(intersect(lab, minfi), aim) #4
intersect(difexp, lab) #5
intersect(minfi, difexp)#6
intersect(intersect(lab, minfi), difexp) #7
intersect(intersect(intersect(lab, minfi), difexp), aim) #7
intersect(lab, cta)
intersect(lab, hervs)
intersect(minfi, cta)
intersect(minfi, hervs)
intersect(aim, cta)
intersect(aim, hervs)
...
length(exp_fc2)
length(exp_fc22)
length(intersect(aim, exp_fc14))
length(intersect(aim, exp_fc22))
