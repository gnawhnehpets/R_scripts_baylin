rm(list=ls())
path <- "/home/steve/.gvfs/onc-analysis$ on onc-cbio2.win.ad.jhu.edu/users/shwang26/methylation_ex/"
path <- "Y:/users/shwang26/methylation_ex"
setwd(path)
...
dir(pattern= as.character(meta$filename[2]))
for(i in 1:nrow(meta)){
  print(i)
  datchar <- c(datchar, dir(pattern=as.character(meta$filename[i])))
}
...
dat <- methylumIDAT(barcodes = filename, idatPath=path)
# ERROR: 60 samples cannot be uploaded
...
names(metadat)
sampleNames(dat) <- paste("colon", metadat$biopsy, sep="_")
sampleNames(dat)
gendat <- dat
datname <- "1to40.txt" #1
