...
target <- readTargets("/home/steve/Desktop/analysis/01142014 Alexandra Borodovsky Agilent GE/targets_full.txt")
RG <- read.maimages(target, source="agilent", path="/home/steve/Desktop/analysis/01142014 Alexandra Borodovsky Agilent GE/")
# save(RG, file="RG")
load("RG")
target
colnames(RG)
...
limma::plotMA3by2(RGb, prefix="postnorm")
getwd()
# head(RGb$G)
# names(RGb)
...
f <- factor(as.character(sub("-AB.*", "", colnames(cydat)))) # for all samples, new.dat
f
pval <- .01
design <- model.matrix(~f)
design
fit <- eBayes(lmFit(log2(cydat), design))
fit
selected  <- fit$p.value[, 2] < pval # or p<.01 #p.adjust(fit$p.value[, 2], method="BY")<.05
esetSel <- cydat[selected,]
dim(esetSel)
head(esetSel)
